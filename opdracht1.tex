\chapter{Klassen en objecten in C++}
...
\begin{itemize}
	\item Selecteer de file \textit{test.cpp} en compileer de files\footnote{compileren lukt niet, controleer of C++ extension (\img{figuren/cPlusIcon} ) is geïnstalleerd} (Terminal $\rightarrow$ Run Build Task... of Ctrl+Shift+B). \textit{(Als je rechtsonder in het venster een melding krijgt over CMake, klik deze dan weg, we gebruiken geen CMake.)}
...
\begin{itemize}
	...
	\item clone de volgende code:\\
	 ...
	 \item Sluit in VSC de huidige folder (\textbf{Ctrl+K F}) en open de folder\\ /home/rock/Documents/opdrLedH/oop/ 
